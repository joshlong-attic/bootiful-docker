...
[source,groovy]
----
include::example/service.groovy[]
----
...
Create an account if you don't already have one and create a new repository. In my case, I named the repository `bootiful_cf`.

Then, we need to tie the local image to the remote repository on Dockerhub. The easiest way to do that is to _tag_ the image, like this:`docker tag bootiful_cf starbuxman/bootiful_cf`. My user is `starbuxman` and I want to link my local image (`bootiful_cf`) to the remote user (`starbuxman`) and remote repository (also called `bootiful_cf`, but it doesn't have to be).

Once that's done, it's dead simple to  upload it: `docker push starbuxman/bootiful_cf`.

Now you can run it with `diego-edge-cli start lattice-app -i "docker:///starbuxman/bootiful_cf" -c "/data/run.sh" `.
